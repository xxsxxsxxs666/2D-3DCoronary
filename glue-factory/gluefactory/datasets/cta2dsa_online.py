...
from  ..utils.vessel_tool import sample_points2, VesselTree, vessel_bound, generate_2d_image_parreral, binary_image_2_points
from ..utils.vessel_projection import cta_2_dsa_2d_wbex, bbox_center_normalize, projection_move_by_lm
import copy
import cv2
...
class _Dataset(torch.utils.data.Dataset):
    ...
    def create_2D3DSample_matching(self, 
                                   cta_3d_points, 
                                   label,
                                   radius,
                                   target_params, 
        ...
        """
        # # Simulating data
        target = cta_2_dsa_2d_wbex(cta_3d_points, target_params)
        grid = cta_2_dsa_2d_wbex(cta_3d_points, grid_params)

        # lm located target and grid | for input
        h, w = 512, 512
        cl_keypoints0, _ = bbox_center_normalize(target, (h, w))
        cl_keypoints1 = projection_move_by_lm(keypoints=grid, lm_512=target[lm_index], lm_index=lm_index)
        ...
        cl_keypoints0, cl_keypoints1 = cl_keypoints0 - np.array([min_w, min_h]), cl_keypoints1 - np.array([min_w, min_h])
        cl_keypoints0_o, cl_keypoints1_o = cl_keypoints0.copy(), cl_keypoints1.copy()  # for cutting


        case_tree, branch_start_end_index = self.vessel_tree.generate_case_tree(label=label, keypoints=cta_3d_points)
        cut_index0 = self.random_cut(label=label, case_tree=case_tree, branch_start_end_index=branch_start_end_index,
        ...
        cut_index1 = self.random_cut(label=label, case_tree=case_tree, branch_start_end_index=branch_start_end_index,
        ...
        target_radius = radius[~cut_index0]
        grid_radius = radius[~cut_index1]

        cl_keypoints0, cl_keypoints1 = cl_keypoints0[~cut_index0], cl_keypoints1[~cut_index1]
        image0 = generate_2d_image_parreral(points_position=cl_keypoints0, radius=target_radius,
                                            image_size=after_padding_size)
        image1 = generate_2d_image_parreral(points_position=cl_keypoints1, radius=grid_radius,
                                            image_size=after_padding_size)
        # skeletonize and sample
        cl2d_keypoints0 = binary_image_2_points(image0, sample_num=sample_num_0)

        # resize image, points
        image0 = cv2.resize(image0, input_size)
        image1 = cv2.resize(image1, input_size)
        scale_factor = np.array(input_size) / np.array(after_padding_size)
        cl_keypoints0, cl_keypoints1 = cl_keypoints0 * scale_factor, cl_keypoints1 * scale_factor
        cl_keypoints0_o, cl_keypoints1_o = cl_keypoints0_o * scale_factor, cl_keypoints1_o * scale_factor
        ...
        gt_matching_dict = self.get_gt_matching(cl_keypoints0_o=cl_keypoints0_o, cl_keypoints1_o=cl_keypoints1_o,
                                                cl_keypoints1=cl_keypoints1, cl2d_keypoints0=cl2d_keypoints0,
                                                cut_index0=cut_index0, cut_index1=cut_index1, cut_index=cut_index,
                                                mode=mode, lax_distance_threshold=lax_distance_threshold)
