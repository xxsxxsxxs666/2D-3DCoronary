...
class VesselDataset(BaseDataset):
    ...
    def _init(self, conf):
        ...
        train_images = [i.parent.relative_to(data_dir).as_posix() for i in train_images]
        train_images = sorted(train_images)  # for deterministic behavior

        val_images = [i.parent.relative_to(data_dir).as_posix() for i in val_images]
...
class _Dataset(torch.utils.data.Dataset):
    ...
    def getitem(self, idx):
        ...
        info = np.load(str(self.image_dir / name / "info.npz"))
        image0_path = self.image_dir / name / "image0.png"
        image1_path = self.image_dir / name / "image1.png"
        image0 = read_image(path=image0_path, grayscale=True)
        image1 = read_image(path=image1_path, grayscale=True)
        ...
        assert image0.max() > 0 and image1.max() > 0, "black picture"
        image0_size = np.array(image0.shape[:2][::-1], dtype=np.float32) / image0.max()
        image1_size = np.array(image1.shape[:2][::-1], dtype=np.float32) / image1.max()
        ...
        """
            new_info = {
        "keypoints0": cl2d_keypoints0,
        "keypoints1": cl_keypoints1,
        "radius1": grid_radius * input_size[0] / 512,  # in 224
        "label1": grid_label,
        "keypoints1_3d": cl_keypoints1_3d,
        "gt_assignment_0": x.astype(np.uint16),
        "gt_assignment_1": y.astype(np.uint16),
        "gt_matches0": gt_matching_dict["gt_matches0"].numpy().astype(np.uint16),
        "gt_matches1": gt_matching_dict["gt_matches1"].numpy().astype(np.uint16),
    }"""
        keypoints0, keypoints1 = info['cl2d_keypoints0'], info['cl_keypoints1']
        keypoints0 = torch.tensor(keypoints0, dtype=torch.float32),
        keypoints1 = torch.tensor(keypoints1, dtype=torch.float32),
        if type(keypoints0) == tuple:
            keypoints0 = keypoints0[0]
        if type(keypoints1) == tuple:
            keypoints1 = keypoints1[0]
        gt_assignment_0 = torch.tensor(info["gt_assignment_0"].astype(np.int16), dtype=torch.long)
        gt_assignment_1 = torch.tensor(info["gt_assignment_1"].astype(np.int16), dtype=torch.long)
        gt_assignment = torch.zeros((keypoints0.shape[0], keypoints1.shape[0]), dtype=torch.float32)
        gt_assignment[gt_assignment_0, gt_assignment_1] = 1
        data = {
            "keypoints0": keypoints0,
            "keypoints1": keypoints1,
            "gt_assignment": gt_assignment,
            "radius1": torch.tensor(info['radius1'], dtype=torch.float32),
            "label1": torch.tensor(info['label1'], dtype=torch.float32),
            "keypoint1_3d": torch.tensor(info['keypoints1_3d'], dtype=torch.float32),
            "gt_matches0": torch.tensor(info['gt_matches0'], dtype=torch.long),
            "gt_matches1": torch.tensor(info['gt_matches1'], dtype=torch.long),
