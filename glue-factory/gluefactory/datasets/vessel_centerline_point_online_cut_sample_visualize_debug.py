...
class _Dataset(torch.utils.data.Dataset):
    ...
    def random_cut(self, label, case_tree, branch_start_end_index: dict,
                   percentage_cut_num=3, percentage_threshold=(0.0, 0.5), p_cut_p=0.5,
                   head_cut_num=3, head_cutting_threshold=(0, 0.5), h_cut_p=0.5,
                   root_cut_num=1, root_cutting_threshold=(0, 0.5), r_cut_p=0.5):
        f = False
        cut_tree = copy.deepcopy(case_tree)
        cut_index = np.zeros(len(label), dtype=bool)
        cut_index_0 = np.zeros(len(label), dtype=bool)
        cut_index_1 = np.zeros(len(label), dtype=bool)
        cut_index_2 = np.zeros(len(label), dtype=bool)
        remove_list_0, remove_percentage_0, remove_list_1, remove_percentage_1 = [], [], [], []
        if percentage_cut_num > 0 and random.random() < p_cut_p:
            ...
            cut_index_0 = cut_index.copy()
            cutting_percentage_0 = cutting_percentage.copy()
            remove_list_0, remove_percentage_0 = remove_list.copy(), remove_percentage.copy()
            f = f
        ...
        if head_cut_num > 0 and random.random() < h_cut_p:
            ...
            cut_index_1 = cut_index_.copy()
            cutting_percentage_1 = cutting_percentage.copy()
            remove_list_1, remove_percentage_1 = remove_list.copy(), remove_percentage.copy()
            f = f
        ...
        if root_cut_num > 0 and random.random() < r_cut_p:
            ...
            cut_index = cut_index | cut_index_
            cut_index_2 = cut_index_.copy()
            f = f

        return cut_index, cut_index_0, cut_index_1, cut_index_2, f, remove_list_0, remove_percentage_0, remove_list_1, remove_percentage_1

    def designate_cut(self, label, case_tree, branch_start_end_index: dict,
                      remove_list, remove_percentage, root_remove_percentage):
        cut_index, cut_index_0, cut_index_1 = np.zeros(len(label), dtype=bool), np.zeros(len(label), dtype=bool), np.zeros(len(label), dtype=bool)
        ...
        if root_remove_percentage > 0:
            cut_index_, cut_tree = self.vessel_tree.random_root_cutting(label=label, case_tree=cut_tree,
                                                                        percentage_threshold=root_remove_percentage)
            cut_index = cut_index | cut_index_
            cut_index_1 = cut_index_.copy()

        return cut_index, cut_index_0, cut_index_1, remove_list, remove_percentage
    ...
    def create_2D3DSample_matching(self, d, mode="single0", sample_num_0=1024, sample_num_1=1024,
        ...
        cut_index0, cut_index0_0, cut_index0_1, cut_index0_2, f1, rm0_0, rmp0_0, rm0_1, rmp1_1 = (
            self.random_cut(label=label_0, case_tree=case_tree, branch_start_end_index=branch_start_end_index,
        ...
        cut_index1, cut_index1_0, cut_index1_1, cut_index1_2, f2, rm1_0, rmp1_0, rm1_1, rmp1_1 = (
            self.random_cut(label=label_0, case_tree=case_tree, branch_start_end_index=branch_start_end_index,
        ...
        cut_index = cut_index0 | cut_index1
        ...
        cl_keypoints1, sample_index = sample_points2(cl_keypoints1_o[~cut_index1], sample_num_1, return_idx=True)
        label1_sample = label_1[~cut_index1][sample_index]
        ...
        data = {
            ...
            "keypoints0_o": torch.tensor(cl_keypoints0_o, dtype=torch.float32),
            "keypoints1_o": torch.tensor(cl_keypoints1_o, dtype=torch.float32),
            "label0": torch.tensor(label_0, dtype=torch.float32),
            "label1": torch.tensor(label_1, dtype=torch.float32),
            "label1_sample": torch.tensor(label1_sample, dtype=torch.float32),
            "radius0": torch.tensor(radius_0, dtype=torch.float32),
            "radius1": torch.tensor(radius_1, dtype=torch.float32),
            "cut_index": torch.tensor(cut_index, dtype=torch.bool),
            "cut_index0": torch.tensor(cut_index0, dtype=torch.bool),
            "cut_index1": torch.tensor(cut_index1, dtype=torch.bool),
            "cut_index0_0": torch.tensor(cut_index0_0, dtype=torch.bool),
            "cut_index0_1": torch.tensor(cut_index0_1, dtype=torch.bool),
            "cut_index0_2": torch.tensor(cut_index0_2, dtype=torch.bool),
            "cut_index1_0": torch.tensor(cut_index1_0, dtype=torch.bool),
            "cut_index1_1": torch.tensor(cut_index1_1, dtype=torch.bool),
            "cut_index1_2": torch.tensor(cut_index1_2, dtype=torch.bool),
            "rm0_0": rm0_0,
            "rmp0_0": rmp0_0,
            "rm0_1": rm0_1,
            "rmp0_1": rmp1_1,
    ...
    def create_simple_data(self, d, mode="single0", sample_num_0=1024, sample_num_1=1024,
                           lax_distance_threshold=0.2):
        """assert: loaded matching is [1, 2, 3, 4....]"""
        """
        :param d:
        ...
        cl_keypoints0_o = d["cl_keypoints0"]
        cl_keypoints1_o = d["cl_keypoints1"]
        cta_3d_point_o = d["cl_keypoints0_3d"]
        label_0 = d["cl_labels0"]
        label_1 = d["cl_labels1"]
        radius_0 = d["cl_radius0"]
        radius_1 = d["cl_radius1"]

        image0 = generate_2d_image_parreral(cl_keypoints0_o, radius_0,
                                            self.conf.image_size)
        image1 = generate_2d_image_parreral(cl_keypoints1_o, radius_1,
                                            self.conf.image_size)

        cl2d_keypoints0 = binary_image_2_points(img=image0, sample_num=sample_num_0)
        cl_keypoints1, sample_index = sample_points2(cl_keypoints1_o, sample_num_1, return_idx=True)
        label1_sample = label_1[sample_index]
        cta_3d_point = cta_3d_point_o[sample_index]

        # 现在3D投影的也需要采样
        gt_matches1_o = np.arange(cl_keypoints1_o.shape[0])
        gt_matches0_o = np.arange(cl_keypoints0_o.shape[0])

        nearest_point_index = np.linalg.norm(cl2d_keypoints0[:, None, :] - cl_keypoints0_o[None, :, :],
                                             axis=-1).argmin(
            axis=1)
        nearest_point_index_2 = np.linalg.norm(cl_keypoints1_o[:, None, :] - cl_keypoints1[None, :, :],
                                               axis=-1).argmin(
            axis=1)
        nearest_point_index_3 = np.linalg.norm(cl_keypoints1[:, None, :] - cl_keypoints1_o[None, :, :],
                                               axis=-1).argmin(
            axis=1)
        gt_matches0_2d_invalid = gt_matches0_o[nearest_point_index] < 0
        gt_matches0_2d = nearest_point_index_2[gt_matches0_o[nearest_point_index]]
        ...
        if mode == "single0":
            ...
            nearest_point_after_lax = cl_keypoints1[y]
            distance_constraint1 = np.linalg.norm(nearest_point_after_lax[:, None, :] -
                                                  nearest_point_after_lax[None, :, :],
                                                  axis=-1) < lax_distance_threshold
            m, n = np.where(distance_constraint1)
            gt2d_assignment = torch.zeros(size=(cl2d_keypoints0.shape[0], cl_keypoints1.shape[0]), dtype=torch.bool)
            gt2d_assignment[torch.tensor(x[m]), torch.tensor(z[n])] = True
        ...
        image0_size = torch.tensor(np.array(image0.shape[:2][::-1], dtype=np.float32))
        image1_size = torch.tensor(np.array(image1.shape[:2][::-1], dtype=np.float32))

        data = {
            "view0": {"image": self.img_to_tensor(image0, return_tensor=True), "image_size": image0_size},
            "view1": {"image": self.img_to_tensor(image1, return_tensor=True), "image_size": image1_size},
            "cta_3d_points": torch.tensor(cta_3d_point),
            "cta_3d_points_o": torch.tensor(cta_3d_point_o),
            "keypoints0": torch.tensor(cl2d_keypoints0, dtype=torch.float32),
            "keypoints1": torch.tensor(cl_keypoints1, dtype=torch.float32),
            "keypoints0_o": torch.tensor(cl_keypoints0_o, dtype=torch.float32),
            "keypoints1_o": torch.tensor(cl_keypoints1_o, dtype=torch.float32),
    ...
    def create_designate_cutting_matching(self, d, mode="single0", sample_num_0=1024, sample_num_1=1024,
                                          lax_distance_threshold=0.2, remove_list_0=None, remove_percentage_0=None, remove_list_1=None, remove_percentage_1=None,
                                          root_remove_percentage_0=0, root_remove_percentage_1=0):
        """assert: loaded matching is [1, 2, 3, 4....]"""
        """
        :param d:
        :param mode: single_0, single_1, bi-directional, bi-directional-lax
        ...
        :return:
        """
        remove_list_0 = [] if remove_list_0 is None else remove_list_0
        remove_percentage_0 = [] if remove_percentage_0 is None else remove_percentage_0
        remove_list_1 = [] if remove_list_1 is None else remove_list_1
        remove_percentage_1 = [] if remove_percentage_1 is None else remove_percentage_1
        ...
        case_tree, branch_start_end_index = self.vessel_tree.generate_case_tree(label=label_0, keypoints=cl_keypoints0_o)
        cut_index0, cut_index0_0, cut_index0_1, rm0_0, rmp0_0 = (
            self.designate_cut(label=label_0, case_tree=case_tree, branch_start_end_index=branch_start_end_index,
                               remove_list=remove_list_0, remove_percentage=remove_percentage_0,
                               root_remove_percentage=root_remove_percentage_0))
        cut_index1, cut_index1_0, cut_index1_1, rm1_0, rmp1_0 = (
            self.designate_cut(label=label_1, case_tree=case_tree, branch_start_end_index=branch_start_end_index,
                               remove_list=remove_list_1, remove_percentage=remove_percentage_1,
                               root_remove_percentage=root_remove_percentage_1))
        cut_index = cut_index0 | cut_index1
        image0 = generate_2d_image_parreral(cl_keypoints0_o[~cut_index0], radius_0[~cut_index0], self.conf.image_size)
        image1 = generate_2d_image_parreral(cl_keypoints1_o[~cut_index1], radius_1[~cut_index1], self.conf.image_size)
