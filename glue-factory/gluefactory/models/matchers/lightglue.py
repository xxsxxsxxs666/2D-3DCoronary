...
def filter_matches_lax(scores: torch.Tensor, kp0: torch.Tensor, kp1: torch.Tensor, th: float, distance_threshold: float):
    """
    obtain lax matches from a log assignment matrix [Bx M+1 x N+1]
    Used for 1toN matching, In this case, we can only make sure that the m0 is correct,
    but not the m1.
    """
    max0, max1 = scores[:, :-1, :-1].max(2), scores[:, :-1, :-1].max(1)
    m0, m1 = max0.indices, max1.indices
    ...
    # mutual0 = indices0 == m1.gather(1, m0)
    """modify the mutual0"""
    kp1_2_kp0 = kp0.gather(1, m1.gather(1, m0).unsqueeze(-1).expand(-1, -1, kp0.shape[-1]))
    distance = torch.linalg.norm(kp1_2_kp0 - kp0, axis=-1)
    mutual0 = (distance < distance_threshold)
    """modify the mutual0"""
    # in this case, we can only make sure that the each point in m0 has a correct match in m1
    mutual1 = indices1 == m0.gather(1, m1)
    max0_exp = max0.values.exp()
    zero = max0_exp.new_tensor(0)
    mscores0 = torch.where(mutual0, max0_exp, zero)
...
class LightGlue(nn.Module):
    ...
    def __init__(self, conf) -> None:
        ...
        if state_dict:
            ...
            # print("loading weights from %s", conf.weights)
    ...
    def forward(self, data: dict) -> dict:
        ...
        # todo: there is some error when using lax mode
        ...
        if self.conf.lax:
            if do_point_pruning:
                assert b == 1, "pruning is only supported for batch size 1"
                m0, m1, mscores0, mscores1 = filter_matches_lax(
                    scores, kpts0.index_select(1, ind0[0]), kpts1.index_select(1, ind1[0]), self.conf.filter_threshold, self.conf.lax_distance_threshold
                )
            else:
                m0, m1, mscores0, mscores1 = filter_matches_lax(
                    scores, kpts0, kpts1, self.conf.filter_threshold, self.conf.lax_distance_threshold
                )
        else:
        ...
        # if self.training and len(all_desc0) > 0:
        #     pred = {
        #         "matches0": m0,
        #         "matches1": m1,
    ...
    def loss(self, pred, data):
        ...
        if not self.training:
            # add metrics
            metrics = matcher_metrics(pred, data, lax=self.conf.lax,
                                      lax_distance_threshold=self.conf.lax_distance_threshold)
        else:
            metrics = {}
