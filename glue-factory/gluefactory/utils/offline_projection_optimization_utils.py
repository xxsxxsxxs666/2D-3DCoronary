...
def cta_2_dsa_rotation_matrix_wbex(angles_in_degrees: np.array):
...
def cta_2_dsa_3d_wbex(heart_center: np.array,
    ...
    num_points = points_cta.shape[0]

    # 1. apply translation
    # heart_center in the shape of [1x3]
    # move the origin to the heart_center
    origin = heart_center
    points_dsa = points_cta - np.tile(origin, (num_points, 1))
   

    # 2. apply rotation
    # get rotation matrix
    rotation_matrix = cta_2_dsa_rotation_matrix_wbex(angles_in_degrees)
    ...
    points_dsa = points_dsa + np.tile([0, 0, SOD], (num_points, 1))
    return points_dsa
...
def optimize_params_extrinsic(CTA_points, DSA_points, param_dict, params_optimize):
    ...
    optimized_result  = optimize.least_squares(CTA_to_DSA_minimize_extrinsic,
            ...
            lower_bound, upper_bound),  jac="3-point", method="dogbox",\
                                               args=(CTA_points,DSA_points,params_optimize,other_params), loss="soft_l1")
...
def metrics_mse(params0, params1, cta_points_3d):
    ...
    mse = np.sqrt(np.square(np.subtract(dsa_points_2d0/params0['DSAImagerSize'][0],\
            dsa_points_2d1/params1['DSAImagerSize'][0])).mean())*params1['DSAImagerSize'][0]*1.414
