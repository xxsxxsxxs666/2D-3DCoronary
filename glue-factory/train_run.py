...
default_train_conf = {
    ...
    "eval_every_iter": 500,  # inrval for evaluation on the validation sette
    "save_every_iter": 3000,  # 5000,  # interval for saving the current checkpoint
    "log_every_iter": 100,  # 200,  # interval for logging the loss to the console
    ...
    "keep_last_checkpoints": 10,  # keep only the last X checkpointsl from a previous experiment
    "load_experiment": None,  # initialize the mode
...
def training(rank, conf, output_dir, args):
    ...
    if args.distributed:
        ...
        device = f"cuda:{args.single_gpu_id}" if torch.cuda.is_available() else "cpu"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--distributed", action="store_true") # 改成分布式
